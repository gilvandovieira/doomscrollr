#!/usr/bin/env bash
# Where does Kysely land — near Hono's +19 (sweet spot) or Drizzle's +462?
# Measures kysely-feed (npm:kysely compiler + @db/postgres) vs jsr-feed (raw @db/postgres) vs
# npm-feed (drizzle + postgres.js), same DB, same query, same session. The kysely-vs-jsr delta
# is Kysely's pure node-compat tax (identical driver). Reuses run.sh's measure() verbatim.
set -u
...
measure(){ local name=$1 file="$HERE/$2"
  ...
  local pid=$!
  until ready; do [ "$(($(date +%s%3N)-t0))" -gt 60000 ] && { echo "  BOOT FAIL"; tail -8 /tmp/jb.log; return; }; [ -d /proc/$pid ] || { echo "  PROC DIED"; tail -8 /tmp/jb.log; return; }; done
...
measure "kysely (npm:kysely compiler + jsr:@db/postgres, clerk floor)" kysely-feed.ts
measure "jsr    (raw jsr:@db/postgres, clerk floor)"                    jsr-feed.ts
measure "npm    (drizzle + postgres.js, clerk floor)"                   npm-feed.ts
echo "done"
