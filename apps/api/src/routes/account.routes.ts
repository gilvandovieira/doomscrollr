...
import { SetUsernameSchema } from "@doomscrollr/shared/schemas/user.schema.ts";
...
accountRoutes.get("/me", async (c) => {
  ...
  if (existing) {
    return c.json({ needsUsername: false, user: await getUserProfile(existing.username) });
  }
  ...
  if (candidate && validateUsername(candidate).ok && !(await isUsernameTaken(candidate))) {
    ...
    return c.json({ needsUsername: false, user: await getUserProfile(created.username) });
  }

  return c.json({ needsUsername: true, user: null });
});
...
accountRoutes.post("/username", async (c) => {
  ...
  if (existing) {
    ...
    return c.json({ needsUsername: false, user: await getUserProfile(updated.username) });
  }
  ...
  return c.json({ needsUsername: false, user: await getUserProfile(created.username) }, 201);
});
